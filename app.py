...
def get_cells_model():
    if "cells_model" not in st.session_state:
        ...
        st.session_state.cells_model = tf.keras.models.load_model("C:\\Users\\mpind\\Downloads\\Streamlit - Copie\\Modeles\\cells_model.keras")
    return st.session_state.cells_model

def get_dog_cat_model():
    if "dog_cat_model" not in st.session_state:
        import tensorflow as tf
        st.session_state.dog_cat_model = tf.keras.models.load_model("C:\\Users\\mpind\\Downloads\\Streamlit - Copie\\Modeles\\dog_cat_model.keras")
...
Ce projet nous a permis de mettre en œuvre des compétences pratiques en Deep Learning, en développement d'application interactive.
    ...
    uploaded_files = st.file_uploader("Uploader une ou plusieurs images de chien ou chat", type=["jpg", "jpeg", "png"], accept_multiple_files=True)
    if uploaded_files:
        ...
        for uploaded_file in uploaded_files:
            ...
            st.image(image_obj, caption="Image uploadée", use_container_width=True)
            ...
            if prediction.shape[1] == 1:
                ...
                if confidence < 0.6:
                    st.error("❌ Veuillez uploader une image correspondant au modèle (chien ou chat). Confiance trop faible.")
                else:
                    st.success(f"🐾 Prédiction : **{predicted_label}** — Confiance : {proba:.2%}")
            else:
                ...
                if confidence < 0.6:
                    ...
                    st.success(f"🐾 Prédiction : **{predicted_label}** — Confiance : {confidence:.2%}")
        ...
        for uploaded_file in uploaded_files:
            ...
            if prediction.shape[1] == 1:
                ...
                if proba > 0.5:
                    ...
                    confidence = 1 - proba
                if confidence < 0.6:
                    st.error("❌ Veuillez uploader une image correspondant au modèle (cellule de paludisme). Confiance trop faible.")
                else:
                    st.success(f"🧪 Prédiction : **{predicted_label}** — Confiance : {confidence:.2%}")
            else:
                predicted_index = int(np.argmax(prediction))
